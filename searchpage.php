...
function searchP(name,position,tname,costL,costU,page,res,sort,order)
	...
	function transfer()
	{
		...
		var check2=playerChecked(document.getElementsByName('searchPlayers'));
		if(!(check1&&check2))
		{
			alert("Error: Please select two players for your transfer!");
			return;
		}
		if(arrayPlayer[check2]==true)
		{
		...
		{
			alert("You can only transfer players of the same position!");
			return;
		}
		if(objS.cells[3].innerText!=objM.cells[3].innerText)
		{
		...
		money=(100*money+100*parseFloat(objM.cells[5].innerText)-100*parseFloat(objS.cells[4].innerText))/100;
		if(money<0)
		{
	...
	var url="transfer.php?pidM="+check1+"&pidS="+check2+"&money="+money;
	if(confirm("Are you sure you want this transfer?"))
	...
	arrayPlayer[check1]=false;arrayPlayer[check2]=true;
	}
	function add(obj)
	{
			...
			<div class="row">
				...
				<div class="masthead"> <!-- Row 2 -->
					...
					<ul class="nav nav-justified">
						...
						<li class="active"><a href="searchpage.php">Transfers</a></li>
...
while($row=mysql_fetch_array($result))
{
	...
	echo "<td><a data-toggle='modal' title='view details of this player' href='#playerinformation' onclick=\"getPlayerInformation('$style')\">" . $row[1] . "</a></td>";
	echo "<td>" . $row[2] . "</td>";
	echo "<td>" . $row[3] . "</td>";
	echo "<td>" . $row[4] . "</td>";
	echo "<td>" . $row[5] . "</td></tr>";
...
?>
					...
					<div class="col-md-2">
							...
							<center>
								<button type="button" class="btn btn-primary custom" href="#" id="swap" name="swap" onclick="transfer()">
								<span class="glyphicon glyphicon-arrow-left"> </span>&nbsp; Swap &nbsp;<span class="glyphicon glyphicon-arrow-right"></span>
								</button> 
							</center>
							<br>
							<br>

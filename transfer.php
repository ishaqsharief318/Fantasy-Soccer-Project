...
$moneyNew=$_GET["money"];
mysql_query("BEGIN"); 
$sql="update consists set playerid=$pidS where uid=$uid and playerid=$pidM";
...
while($row=mysql_fetch_array($result))
{
	...
	echo "<td id='$row[9]'><a data-toggle='modal' title='view details of this player' href='#playerinformation' onclick='javascript:getPlayerInformation(\"$style\")'>" . $row[1] . "</a></td>";
	echo "<td>" . $row[2] . "</td>";
	echo "<td>" . $row[3] . "</td>";
	echo "<td>" . $row[4] . "</td>";
	echo "<td>" . $row[5] . "</td></tr>";
